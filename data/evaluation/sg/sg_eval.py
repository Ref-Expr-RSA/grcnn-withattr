...
def do_sg_evaluation(dataset, predictions, predictions_pred, output_folder, logger):
    ...
    for mode in modes:
        ...
        for image_id, (prediction, prediction_pred) in enumerate(zip(predictions, predictions_pred)):
            ...
            #print(len(pred_entry['pred_classes']),len(pred_entry['pred_rel_inds']),len(pred_entry['obj_scores']),torch.argmax(torch.tensor(pred_entry['rel_scores']),dim=1))
            #print((pred_entry['pred_classes']),(pred_entry['pred_rel_inds']),pred_entry['obj_scores'],pred_entry['rel_scores'])
...
def evaluate(gt_classes, gt_boxes, gt_rels,
             ...
             top_Ns, result_dict,
             mode, iou_thresh=0.5):
    #print('eva',len(gt_classes), len(gt_rels),
     #         len(obj_labels),
      #       len(rel_inds)
       #     )
   # print('eva',gt_classes, gt_rels,
    #          obj_labels,
     #        rel_inds
      #      )
