...
class PostProcessor(nn.Module):
    ...
    def forward(self, x, boxes, skip_nms=False):
        ...
        class_logit, box_regression ,attr_logits= x
        class_prob = F.softmax(class_logit, -1)
        atts_prob=F.softmax(attr_logits,-1)
        ...
        # ):
            ...
            if not self.bbox_aug_enabled and not skip_nms:  # If bbox aug is enabled, we will do it later
                ...
                boxlist = self.prepare_boxlist(boxes_per_img, features_per_img, prob, logit, image_shape,attr_per_img,atts_prob)
                ...
                if not self.relation_on:
                    ...
                    while len(boxlist_filtered) < self.min_detections_per_img:
                        ...
                        boxlist_filtered = self.filter_results_nm(boxlist, num_classes,num_atts, thresh=score_thresh)
            else:
                ...
                boxlist.add_field("atts", boxes[idx].get_field("atts"))
                boxlist.add_field("atts_prob", boxes[idx].get_field("atts_prob"))
                # print('addatts')
    ...
    # def prepare_boxlist(self, boxes, features, scores, logits, image_shape):
        ...
        boxlist.add_field("atts",attr)
        boxlist.add_field("logits", logits)
        boxlist.add_field("atts_prob", atts_prob)
    ...
    def filter_results_nm(self, boxlist, num_classes, num_atts,thresh=0.05):
        """Returns bounding-box detection results by thresholding on scores and
        ...
        scores = boxlist.get_field("scores").reshape(-1, num_classes)
        logits = boxlist.get_field("logits").reshape(-1, num_classes)
        atts = boxlist.get_field("atts").reshape(-1, num_atts)
        atts_prob = boxlist.get_field("atts_prob").reshape(-1, num_atts)
        features = boxlist.get_field("features")

        valid_cls = (scores[:, 1:].max(0)[0] > thresh).nonzero() + 1
        valid_atts = (atts_prob[:, 1:].max(0)[0] > thresh).nonzero() + 1

        nms_mask = scores.clone()
        nms_atts_mask = atts_prob.clone()
        nms_mask.zero_()
        nms_atts_mask.zero_()
        ...
        for j in valid_cls.view(-1).cpu():
            ...
            result.append(boxlist_for_class)
        result=[]
        for j in valid_atts.view(-1).cpu():
            scores_j = atts_prob[:, j]
            boxes_j = boxes[:, j * 4: (j + 1) * 4]
            boxlist_for_class = BoxList(boxes_j, boxlist.size, mode="xyxy")
            boxlist_for_class.add_field("atts_prob", scores_j)
            boxlist_for_class.add_field("idxs", torch.arange(0, scores.shape[0]).long())
            # boxlist_for_class = boxlist_nms(
            #     boxlist_for_class, self.nms
            # )
            boxlist_for_class = boxlist_nms(
                boxlist_for_class, 0.3,score_field='atts_prob'
            )
            nms_atts_mask[:, j][boxlist_for_class.get_field("idxs")] = 1

            num_labels = len(boxlist_for_class)
            boxlist_for_class.add_field(
                "labels", torch.full((num_atts,), j, dtype=torch.int64, device=device)
            )
            result.append(boxlist_for_class)

        dists_all = nms_mask * scores
        attr_all = nms_atts_mask * atts_prob

        # filter duplicate boxes
        scores_pre, labels_pre = dists_all.max(1)
        scores_attr_pre, labels_attr_pre = attr_all.max(1)
        ...
        result.add_field("atts", atts_all)
