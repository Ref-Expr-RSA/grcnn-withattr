...
class PostProcessor(nn.Module):
    ...
    def forward(self, x, boxes, skip_nms=False):
        ...
        attr_prob=F.softmax(attr_logits,-1)
        ...
        num_atts=attr_prob.shape[1]
        attr_logits = attr_logits.split(boxes_per_image, dim=0)
        attr_prob = attr_prob.split(boxes_per_image, dim=0)

        results = []; idx = 0
        for prob, logit, boxes_per_img, features_per_img, image_shape,attr_per_img,attr_prob in zip(
            class_prob, class_logit, proposals, features, image_shapes,attr_logits,attr_prob
        ):
        ...
        # ):
            ...
            if not self.bbox_aug_enabled and not skip_nms:  # If bbox aug is enabled, we will do it later
                ...
                boxlist = self.prepare_boxlist(boxes_per_img, features_per_img, prob, logit, image_shape,attr_per_img,attr_prob)
                ...
                boxlist.add_field("attr_prob", boxes[idx].get_field("atts"))
                # print('addatts')
    ...
    def prepare_boxlist(self, boxes, features, scores, logits, image_shape,attr,attr_prob):
    # def prepare_boxlist(self, boxes, features, scores, logits, image_shape):
        ...
        boxlist.add_field("atts_prob", attr_prob)
