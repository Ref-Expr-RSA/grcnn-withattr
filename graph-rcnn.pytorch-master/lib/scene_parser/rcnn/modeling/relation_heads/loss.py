...
class FastRCNNLossComputation(object):
    ...
    def match_targets_to_proposals(self, proposal, target):
        ...
        # target_pair.add_field("atts", target.get_field("pred_labels").view(-1))
    ...
    def obj_classification_loss(self, proposals, class_logits):
        ...
        # print(class_logits.shape, labels.shape)
        # print(class_logits, labels)
        # print(class_logits.shape,labels.shape,'label')
        return classification_loss
    def attr_classification_loss(self, proposals,class_logits):
        class_logits = cat(class_logits, dim=0)

        device = class_logits.device

        labels = cat([proposal.get_field("atts") for proposal in proposals], dim=0)
        # print(class_logits.shape,labels.shape)
        # print(class_logits,labels)
        # import numpy as np
        # class_logits=torch.tensor(np.random.random((512, 51))).to(device)
        classification_loss = F.cross_entropy(class_logits, labels)
        # print(classification_loss,'attr-loss')
        return classification_loss
