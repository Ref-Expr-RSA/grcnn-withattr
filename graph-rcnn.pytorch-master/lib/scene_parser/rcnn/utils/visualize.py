...
def overlay_class_names(image, predictions, categories,ind_to_att):
    """
    ...
    atts = predictions.get_field("atts").tolist()
    # print(atts)
    # print(labels)
    # print((ind_to_att))
    atts_label=[]
    for i in atts:
        if  i!=0:
            atts_label.append(ind_to_att[i])
        else:
            atts_label.append('noattr')
    # atts = [ind_to_att[i] for i in atts]
    atts=atts_label
    ...
    for box, score, label,atts in zip(boxes, scores, labels,atts):
        label=label if atts=='noattr' else atts+ ' '+ label
        x, y = box[:2]
