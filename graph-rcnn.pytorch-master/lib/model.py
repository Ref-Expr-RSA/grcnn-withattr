...
class SceneGraphGeneration:
    ...
    def __init__(self, cfg, arguments, local_rank, distributed):
        ...
        # print(type(self.data_loader_train.dataset.ind_to_classes),self.data_loader_train.dataset.ind_to_classes)
        # print(self.data_loader_train.dataset.ind_to_classes)
        # print(self.data_loader_train.dataset.ind_to_att)
        cfg.DATASET.IND_TO_ATTR = self.data_loader_train.dataset.ind_to_att
    ...
    def _get_freq_prior(self, must_overlap=False):
        ...
        for ex_ind in range(len(self.data_loader_train.dataset)):
            ...
            # gt_attrs = self.data_loader_train.dataset.gt_att[ex_ind].copy()
    ...
    def train(self):
        ...
        for i, data in enumerate(self.data_loader_train, start_iter):
            ...
            # print(len(targets.get_field('atts')))
            ...
            # w=open('param.txt','w')
            # for name, param in self.scene_parser. named_parameters():
            #    w.write(name+str(True if param.grad is not None else False)+"\n")
            ...
            if (i + 1) % self.cfg.SOLVER.CHECKPOINT_PERIOD == 0:
                self.sp_checkpointer.save("checkpoint_roi_with_attr_{:07d}".format(i), **self.arguments)
            if (i + 1) == max_iter:
                self.sp_checkpointer.save("checkpoint_roi_with_attr_final", **self.arguments)
    ...
    def visualize_detection(self, dataset, img_ids, imgs, predictions):
        ...
        for i, prediction in enumerate(predictions):
            ...
            # img = imgs.tensors[i].cpu().numpy()
            result = img.copy()
            result = overlay_boxes(result, top_prediction)
            result = overlay_class_names(result, top_prediction, dataset.ind_to_classes,dataset.ind_to_att)
            # cv2.imwrite(os.path.join(visualize_folder, "detection_{}.jpg".format(img_ids[i])), result)
            cv2.imwrite(os.path.join(visualize_folder, "detection_{}_grcnn.jpg".format(img_ids[i])), result)
            # cv2.imwrite(os.path.join(visualize_folder, "detection_{}_attrgrcnn.jpg".format(img_ids[i])), result)
