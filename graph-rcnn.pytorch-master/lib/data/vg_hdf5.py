...
class vg_hdf5(Dataset):
    def __init__(self, cfg, split="train", transforms=None, num_im=-1, num_val_im=5000,
        ...
        self.att_to_ind=self.info['attribute_to_idx']
        # self.ind_to_att=self.info['idx_to_attribute']
        self.ind_to_classes = sorted(self.class_to_ind, key=lambda k:
                               self.class_to_ind[k])
        # print(type(self.class_to_ind),type(self.att_to_ind))
        self.ind_to_att = sorted(self.att_to_ind, key=lambda k:
                               self.att_to_ind[k])
        # print(len(self.ind_to_att))
        # print(len(self.att_to_ind))
        ...
        # print(len(self.ind_to_classes),self.ind_to_classes[151])
        #print(filter_non_overlap)
        #print(split)
        #filter_non_overlap=np.logical_and(filter_non_overlap==True,split == "train")
        #print(filter_non_overlap)
        self.split_mask, self.image_index, self.im_sizes, self.gt_boxes, self.gt_classes, self.relationships ,self.gt_attributes= load_graphs(
    ...
    def __getitem__(self, index):
        ...
        obj_attrs=self.gt_attributes[index].copy()
        ...
        # print(len(obj_attrs))
        target.add_field("atts", torch.from_numpy(obj_attrs))
...
def load_graphs(graphs_file, images_file, mode='train', num_im=-1, num_val_im=0, filter_empty_rels=True,
    ...
    # get attribute
    all_attribute=roi_h5['attribute'][:,0]
    # print(all_attribute,all_labels)
    ...
    for i in range(len(image_index)):
        ...
        gt_attribute_i=all_attribute[im_to_first_box[i]:im_to_last_box[i] + 1]
        ...
        gt_attributes.append(gt_attribute_i)
        relationships.append(rels)

    im_sizes = np.stack(im_sizes, 0)
    # print(len(gt_attributes),len(gt_classes))
    return split_mask, image_index_valid, im_sizes, boxes, gt_classes, relationships,gt_attributes
