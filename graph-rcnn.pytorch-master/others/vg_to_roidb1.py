...
def preprocess_attribute_labels(data, alias_dict={}):
    for img in data:
        # print(img)
        for at in img['attributes']:
            att_label=[]
            # print(at)
            if 'attributes' in at:
                for iter_at in at['attributes']:
                    label = sentence_preprocess(iter_at)
                    att_label.append(label)
                # print(att_label)
                at['attributes'] = att_label
...
def extract_predicate_token(data, num_tokens, pred_list=[], verbose=True):
    ...
    if verbose:
        print(('Keeping %d / %d predicates with enough instances'
                  % (len(tokens), len(token_counter))))
    return tokens, token_counter_return
def extract_attribute_token(data, num_tokens, pred_list=[], verbose=True):
    """ Builds a set that contains the relationship predicates. Filters infrequent tokens. """
    token_counter = Counter()
    total = 0
    for img in data:
        for at in img['attributes']:
            if 'attributes' in at:
                for iter_ob in at['attributes']:
                    if iter_ob in common_attributes:
                        token_counter.update([iter_ob])
    tokens = set()
    token_counter_return = {}
    for token, count in token_counter.most_common():
        tokens.add(token)
...
def build_token_dict(vocab):
    ...
    for token in vocab_sorted:
        ...
        next_idx = next_idx + 1
    # token_to_idx['noattr']=next_idx
    # idx_to_token[next_idx]='noattr'
...
def encode_objects(obj_data, token_to_idx, token_counter, org_h, org_w, im_long_sizes):
    ...
    for i, img in enumerate(obj_data):
        ...
        for obj in img['objects']:
           # pick a label for the object
            ...
            if obj_label is not None:
                ...
                # print(encode_box[im_long_sizes[0]])
                encoded_labels.append(token_to_idx[obj_label])
                encoded_obj_id_small.append(obj['object_id'])
                # print(token_to_idx[obj_label])
                for obj_id in obj['ids']: # assign same index for merged ids
                    img['id_to_idx'][obj_id] = obj_counter

                obj_counter += 1
        encoded_obj_id.append(encoded_obj_id_small)
        ...
        # print(im_to_first_obj[i],im_to_last_obj[i])

    for k, boxes in encoded_boxes.items():
        #print(np.vstack(boxes))
        encoded_boxes[k] = np.vstack(boxes)
    return np.vstack(encoded_labels), encoded_boxes, im_to_first_obj, im_to_last_obj,encoded_obj_id
...
def encode_attributes(attri_data, attribute_to_idx, encoded_obj_id):
    att_all=[]
    count=0
    for i,at in enumerate(attri_data):
        obj_per_img=[]
        id_to_idx = encoded_obj_id[i]
        for iter_obj in id_to_idx:
            object_attr=[]
            count+=1
            for iter_attr in at['attributes']:
                # print(iter_attr,iter_obj,len(id_to_idx))
                if iter_attr['object_id']==iter_obj and 'attributes' in iter_attr:
                    for itr_obj_attr in iter_attr['attributes']:
                        if itr_obj_attr in attribute_to_idx :
                            if len(object_attr)==0:
                                      object_attr=[attribute_to_idx[itr_obj_attr]]
                            else:
                                object_attr=[attribute_to_idx[itr_obj_attr]] if obj_per_img.count([attribute_to_idx[itr_obj_attr]])< obj_per_img.count(object_attr) else object_attr
            if len(object_attr)==0:
                object_attr=[attribute_to_idx['noattr']]
            obj_per_img+=object_attr

        att_all+=obj_per_img

    # print(att_all)
    # print(len(att_all),len(encoded_obj_id))
    return np.vstack(np.array(att_all))
...
def sync_attribute(obj_data,attri_data):
    num_img = len(obj_data)
    for i in xrange(num_img):
        assert(obj_data[i]['image_id'] == attri_data[i]['image_id'])
        objs = obj_data[i]['objects']
        atts = attri_data[i]['attributes']

        ids = [obj['object_id'] for obj in objs]
        for at in atts:
            if at['object_id'] not in ids:
                at_obj = at
                objs.append(at_obj)
        obj_data[i]['objects'] = objs
def main(args):
    ...
    attri_data=json.load(open(args.attribute_input))
    assert(len(rel_data) == len(obj_data) and
           len(obj_data) == len(img_data)) and  len(attri_data)==len(obj_data)
    ...
    attri_data = filter_by_idx(attri_data, valid_im_idx)
    # print(len(obj_data),len(obj_data[0]))
    # obj_data=obj_data[:10]
    # print(len(obj_data), len(obj_data[0]))
    # rel_data=rel_data[:10]
    # img_data=img_data[:10]
    # attri_data=attri_data[:10]
    # num_im=10
    # sanity check
    for i in xrange(num_im):
        assert(obj_data[i]['image_id'] \
               == rel_data[i]['image_id'] \
               == img_data[i]['image_id'] \
               == img_ids[i]==attri_data[i]['image_id']
               )
    ...
    preprocess_attribute_labels(attri_data, alias_dict=pred_alias_dict)
    ...
    attribute_tokens, attribute_token_counter = extract_attribute_token(attri_data,
                                                                        args.num_attributes,
                                                                        None)
    attribute_to_idx, idx_to_attribute = build_token_dict(attribute_tokens)
    attribute_to_idx['noattr']=0
    idx_to_attribute[0]='noattr'
    # print(label_to_idx, idx_to_label )
    # print( attribute_to_idx, idx_to_attribute )
    ...
    # encode object
    encoded_label, encoded_boxes, im_to_first_obj, im_to_last_obj ,encoded_obj_id= \
    encode_objects(obj_data, label_to_idx, object_token_counter, \
                   heights, widths, img_long_sizes)
    # print(len(encoded_obj_id),len(encoded_label),encoded_obj_id)
    # print(encoded_label, encoded_boxes, im_to_first_obj, im_to_last_obj )
    ...
    # print(len(attri_data),len(obj_data))
##attribute
    encoded_attr = \
        encode_attributes(attri_data, attribute_to_idx, encoded_obj_id)
    # print(len(encoded_label),len(encoded_attr))
    # for iter_label in range(len(encoded_label)):
        # print(idx_to_label[encoded_label[iter_label][0]])
        # print(idx_to_attribute[encoded_attr[iter_label][0]])
    # print(encoded_label)
    f.create_dataset('attribute', data=encoded_attr)


    # relationship
    encoded_predicate, encoded_rel, im_to_first_rel, im_to_last_rel = \
    encode_relationships(rel_data, predicate_to_idx, obj_data)
    # print(encoded_predicate, encoded_rel, im_to_first_rel, im_to_last_rel)
   ...
   # build train/val/test splits
    ...
    json_struct = {
        ...
        'attribute_token_counter': attribute_token_counter
    }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--attribute_input', default='VG/attributes.json', type=str)
    ...
    parser.add_argument('--num_attributes', default=50, type=int, help="set to 0 to disable filtering")
